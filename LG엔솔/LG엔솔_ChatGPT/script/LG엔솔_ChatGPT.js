...
function resGptAllText(text, gubun) {
    ...
    var messageCopyTooltip = $('<div class="f-tooltip">'+answerMsg+' 답변 전체 복사</div>');
    ...
    copyButton.on('click', function() {
        ...
        showConfirmDialog(answerMsg+' 답변을 복사했어요!<br />원하는 창에 붙여넣기 해주세요.');
    });
...
function chkSecureText(reqText){
  ...
  var reqParam = {"query":{"text":"" , "languageCode":"ko"}};
  var sessionId = chatui.getSessionId();
  var reqHeader = {};
...
//   $(".test-panel .panel-wrapper .chat-panel .form-group").append('<input type="text" class="sendText form-control test-sentence-input caas-chat-input-back-color caas-chat-input-font-color ui-autocomplete-input" placeholder="질문을 입력해주세요" autocomplete="off">');
  $(".test-panel .panel-wrapper .chat-panel .form-group").append('<textarea type="text" style="resize:none" class="sendText form-control test-sentence-input caas-chat-input-back-color caas-chat-input-font-color ui-autocomplete-input" placeholder="메시지를 입력해주세요 (줄바꿈은 Shift + Enter)"></textarea>');
...
//     $('#caas-chatbot').append(firstSlide);
  ...
  chatbotCollapse.addEventListener('click', function(e) {
    pop.open('create', $(this), 'Gpt_Bot_Close', 'loadEl.gpt_bot_close("ChatGPT Mode 종료","종료 시 대화 내용이 저장되지 않고<br />업무 챗봇으로 돌아갑니다.<br />종료할까요?")');
  ...
  $("ul.menu-list > li > a").removeAttr('href');
  
  $("ul.menu-list > li > a").each(function(index) {  
      
      $(this).on('click', function(e) {
          console.log('quick link value : '+index);

        searchActiveTrue(index);
  ...
  function searchActiveTrue(quickIdx) {
    ...
    switch(quickIdx) {
        case 0:
            searchType = "B-LEX";
            $('.form-group').append('<span class="search-pre">/B-LEX </span>');
            $('.sendText').addClass('search-active-blex');
            break;
        default:
            break;
    }
  ...
  function searchActiveFalse() {
      ...
      $('.search-pre').remove();
      $('.sendText').removeClass('search-active-blex');
      $('.sendText').val("");
      $('.sendText').attr("placeholder", "메시지를 입력해 주세요.");    
  ...
  setTimeout(function() {
    ...
    if(resp.response.query.event == "WELCOME") {
      if(chatui.getParameter("queryText")) {
          ...
          chatui.sendEventMessage("callGPT",{"reqText":chatui.getParameter("queryText")});
      ...
      if(message.quickReply && message.quickReply.length > 0) {
        ...
        for(var k=0; k<message.quickReply.length; k++) {
          ...
          if(button.type == 'btn') {
            ...
            appendQueryText(mText);
            chatui.sendEventMessage("callGPT",{"reqText":mText});
...
chatui.createCustomResponseMessage = function(resp, isHistory) {
  ...
  if(resp.response) {
    ...
    if(customPayload.type == 'gptPush') {
        ...
        var checkContents;
        var messages = $('<div class="message caas-chat-response-message-back-color caas-chat-response-message-font-color">ChatGPT의 답변입니다.</div>');
        ...
        var statusMessageCopy = $('<div class="copy-question"></div>');
        var messageCopyTooltip = $('<div class="f-tooltip">ChatGPT 답변 전체 복사</div>');
        statusMessageCopy.append(messageCopyTooltip);
        var copyButton = $('<button type="button" class="btn-text btn-emphasis">'
        +'<svg width="17" height="16" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg">'
        +'<path fill-rule="evenodd" clip-rule="evenodd" d="M6.83325 1C5.72868 1 4.83325 1.89543 4.83325 3C3.72868 3 2.83325 3.89543 2.83325 5V13C2.83325 14.1046 3.72868 15 4.83325 15H10.1666C11.2712 15 12.1666 14.1046 12.1666 13C13.2712 13 14.1666 12.1046 14.1666 11V3C14.1666 1.89543 13.2712 1 12.1666 1H6.83325ZM12.1666 12.2C12.8293 12.2 13.3666 11.6627 13.3666 11V3C13.3666 2.33726 12.8293 1.8 12.1666 1.8H6.83325C6.17051 1.8 5.63325 2.33726 5.63325 3H10.1666C11.2712 3 12.1666 3.89543 12.1666 5L12.1666 12.2ZM3.63325 5C3.63325 4.33726 4.17051 3.8 4.83325 3.8H10.1666C10.8293 3.8 11.3666 4.33726 11.3666 5V13C11.3666 13.6627 10.8293 14.2 10.1666 14.2H4.83325C4.17051 14.2 3.63325 13.6627 3.63325 13V5Z" fill="#12a6be"/>'
        ...
        copyButton.on('click', function() {
            ...
            showConfirmDialog('ChatGPT 답변을 복사했어요!<br />원하는 창에 붙여넣기 해주세요.');
        });
        ...
        //   chatui.sendEventMessage("btnCallGPT",{"reqText":reqMsg,"keyWord":keyword});
          chatui.sendEventMessage("callGPT",{"isProblem":"btn","reqText":reqMsg,"keyWord":keyword});
        });
      ...
      showCheckPersonCharge();
    ...
    } else if(customPayload.type == 'blexpush') {    
        
        console.log('resp.message >>> '+resp.message);
        
        var viewLimit = 200;
        var checkContentsText = resp.message;
        var checkContents;
        var messages = $('<div class="message caas-chat-response-message-back-color caas-chat-response-message-font-color">B-LEX의 답변입니다.</div>');
        ...
        var statusMessageCopy = $('<div class="copy-question"></div>');
        var messageCopyTooltip = $('<div class="f-tooltip">B-LEX 답변 전체 복사</div>');
        statusMessageCopy.append(messageCopyTooltip);
        var copyButton = $('<button type="button" class="btn-text btn-emphasis">'
        +'<svg width="17" height="16" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg">'
        +'<path fill-rule="evenodd" clip-rule="evenodd" d="M6.83325 1C5.72868 1 4.83325 1.89543 4.83325 3C3.72868 3 2.83325 3.89543 2.83325 5V13C2.83325 14.1046 3.72868 15 4.83325 15H10.1666C11.2712 15 12.1666 14.1046 12.1666 13C13.2712 13 14.1666 12.1046 14.1666 11V3C14.1666 1.89543 13.2712 1 12.1666 1H6.83325ZM12.1666 12.2C12.8293 12.2 13.3666 11.6627 13.3666 11V3C13.3666 2.33726 12.8293 1.8 12.1666 1.8H6.83325C6.17051 1.8 5.63325 2.33726 5.63325 3H10.1666C11.2712 3 12.1666 3.89543 12.1666 5L12.1666 12.2ZM3.63325 5C3.63325 4.33726 4.17051 3.8 4.83325 3.8H10.1666C10.8293 3.8 11.3666 4.33726 11.3666 5V13C11.3666 13.6627 10.8293 14.2 10.1666 14.2H4.83325C4.17051 14.2 3.63325 13.6627 3.63325 13V5Z" fill="#12a6be"/>'
        ...
        copyButton.on('click', function() {
            ...
            showConfirmDialog('B-LEX 답변을 복사했어요!<br />원하는 창에 붙여넣기 해주세요.');
        });
        ...
        seeMore.on('click', function() {
          $(this).parents('.request-status').find('.list-box').removeClass('disp-none');
          resGptAllText(checkContentsText, 'B-LEX');
        //   $(this).remove();
        });

        //checkContentsText.length>viewLimit?checkContents.append(seeMore):checkContents.append(statusMessageCopy.append(copyButton));
        checkContents.append(seeMore);
        ...
        var sendMessageBtn = $('<span class="btn-custom-reply send-message" data-cmessage="B-LEX 문서결과">' + '관련 B-LEX 문서 보러가기 ' + '</span>');
        sendMessageBtn.on('click', function() {
          var mText = $(this).data('cmessage');
            console.log('btn mtext : '+mText);
        //appendQueryText(mText);
        //chatui.sendEventMessage("callGPT",{"reqText":mText});
        });
