...
function chkSecureText(reqText){
  ...
  var limitSize = 256;
  var cntSize = parseInt(utterance.length/limitSize)+1;
  var lastSize = utterance.length%limitSize;
  ...
  sessionId && (reqHeader["X-CHATBOT-SESSION"] = sessionId);
    ...
    for(var i=0; i < cntSize; i++){
        ...
        var resutltMessage;
        var reqText;
        if(i==0) reqText = utterance.substr(0,limitSize);
        else if(i==cntSize-1) reqText = utterance.substr(i*limitSize,cntSize*limitSize+lastSize);
        else reqText = utterance.substr(i*limitSize,limitSize);
        reqParam.query.text = reqText;
        // console.log("reqParam : "+reqText);
        ...
        $.ajax({
    	    ...
    	    , success: function(payload, textStatus, jqXHR) {
                console.log(payload);
                nameByPayLoad = payload.queryResult.intent.name;
                resutltMessage = payload.queryResult.text;
    	    }
        });
        
        if(nameByPayLoad !== "Default Fallback Intent"){
            showConfirmDialog(resutltMessage);
            return;
        }
    }
}
...
//   $('#divScroll').append(beforeConv);
  ...
  $('.sendText').on('keyup', function(e) {
    ...
    if(val.length > 0 && (e.code == 'Enter' || e.keyCode == 13)) {
        ...
        if(!e.shiftKey){
            var textEnt = $(this).val().substr(0,val.length-1);
            console.log("text : "+textEnt);
            $('.sendText').val(textEnt);
            resizeTA(this);
            ...
            setTimeout(function() {
              ...
              var sessionId = chatui.getSessionId();
              appendQueryText(val);
              chatui.sendEventMessage("callGPT",{"reqText":val});
              $('.sendText').val('');
              $('.btn-send').removeClass('active');
        }
    }
